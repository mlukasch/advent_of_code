...
let inputString = "d dec 461 if oiy <= 1
...
type number = int;;
type command = number * number -> int;;
...
let parseCondition condString: condition =
  ...
  let register = String.trim (List.hd tokens) in
...
let parseInstruction instructionString: instruction =
  ...
  let register = String.trim (List.hd subTokens) in
...
let executeInstruction hashtbl instruction : unit =
  ...
  if (condition hashtbl) then (
    let value = if Hashtbl.mem hashtbl register then Hashtbl.find hashtbl register else 0 in
    let result = command (value,number) in
    (
      Printf.printf "result : %d , register : %s, val: %d, number : %d\n" result register value number;
      Hashtbl.remove hashtbl register;
...
let findLargestNumber  (hashtbl : (register, int) Hashtbl.t) : int =
  ...
  (
    Hashtbl.iter 
      (fun k v -> Printf.printf "k: %s | v: %d\n" k v;if v > !result then result := v else ()) 
      hashtbl;
    !result
  );;

#trace findLargestNumber
...
let maxVal = parseInput inputString |> executeInstructionList |> findLargestNumber;;
