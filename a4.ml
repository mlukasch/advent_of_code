#load "str.cma";;
(*
aa bb cc dd is valid.
aa bb cc dd aa is not valid - the word aa appears more than once.
aa bb cc dd aaa is valid - aa and aaa count as different words.
*)

let splitWords sentence = 
  let regex = Str.regexp "[ \t]+" in 
  let results = Str.split regex sentence in
  results;;

let hasDuplicates words = 
  let store = Hashtbl.create 1 in
  (
    let rec aux w = match w with
      | [] -> false
      | h::t -> if (Hashtbl.mem store h) then true else (Hashtbl.add store h true; aux t)
    in aux words;
  );;

let logger = List.map (fun el -> (Printf.printf "| %s |" el; el));;

let check sentence =  sentence |> splitWords |> logger |> hasDuplicates |> not;;

[
  "aa bb cc dd ee";
  "aa bb cc dd aa";
  "aa bb cc dd aaa"
] |> List.iter (fun test -> Printf.printf "check for duplicates: %s -> result : %b\n" test (check test));;
...
let collector seq = 
  let rec aux acc s = match s with
    | [] -> acc
    | h::t -> if h then aux (acc + 1) t else aux acc t
  in aux 0 seq;;
