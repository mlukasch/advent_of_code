...
let parseInput inputString : disc list =
...
let findRootNode discList =
  let allChildKeys = ref [] in
  let discKeys = ref [] in
  (
    List.iter (
      fun disc -> match disc with
        | { key; children=Some childKeys } -> 
          allChildKeys := childKeys@(!allChildKeys);
          discKeys := key::!discKeys;
          ();
        | _ -> ();
    ) discList;
    let rootList = List.filter (fun el -> not(List.mem el !allChildKeys)) !discKeys in
    List.hd rootList
  );;

let rootKey = 
  let discList = parseInput inputString in
  findRootNode discList;;
