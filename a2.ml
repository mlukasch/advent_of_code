...
let convertInput inputString =
  let splitLines text = Str.split (Str.regexp "\n") text in
  let splitTokens line = Str.split (Str.regexp "[ \t]+") line in
  let convertToken token = int_of_string token in
  inputString |> splitLines |> List.map (fun line -> line |> splitTokens |> List.map (fun token -> Printf.printf "|%s|" token; token) |> List.map convertToken);;


let checkSum checkSumInput = 
  let rec getMin acc (row: int list) = match row with
    | [] -> acc
    | h::t -> if h > acc then (getMin acc t) else (getMin h t) in
  let rec getMax acc (row: int list) = match row with
    | [] -> acc
    | h::t -> if h > acc then (getMax h t) else (getMax acc t) in
  let calcDiffInRow (row: int list) = 
    Printf.printf "\nmax: %d" (getMax min_int row); 
    Printf.printf "\nmin: %d" (getMin max_int row); 
    (getMax min_int row) - (getMin max_int row) in
  let rec calcCheckSum acc (table: int list list) = match table with
    | [] -> acc
    | h::t -> Printf.printf "\ndiff: %d|" (calcDiffInRow h); calcCheckSum (acc + calcDiffInRow h) t in
  calcCheckSum 0 checkSumInput;;

let result = (inputString |> convertInput |> checkSum) in
